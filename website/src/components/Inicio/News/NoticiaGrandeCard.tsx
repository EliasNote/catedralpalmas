...
export default function NoticiaGrandeCard({ news }: { news: News[] }) {
  ...
  useEffect(() => {
    if (hovered || news.length <= 1) return;

    const interval = setInterval(next, 5000);
    return () => clearInterval(interval);
  }, [hovered, news.length, next]);
  ...
  // Se não há notícias, não renderiza nada
  if (!news || news.length === 0 || !activeNews) {
    return null;
  }

  const formattedDate = formatDate(new Date(activeNews.published_at));
  ...
  return (
    ...
    >
      ...
      <div className="p-10 absolute bottom-0 text-left max-w-[620px] flex flex-col gap-4 z-10 pointer-events-none">
        <h3 className="text-lg text-white text-left mb-0">
          {activeNews.title}
        </h3>
        <p className="text-md text-white">{activeNews.summary}</p>
